...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  return arr.filter(statistic => statistic.baseStat > minBaseStat);
};
...
const getStatName = (arr, minBaseStat) => {
  let newArr = [];
  arr.forEach(statistic => {
    if(statistic.baseStat > minBaseStat) {
      newArr.push(statistic.stat.name);
    }
  });
  return newArr;
};
...
const evenOddNumericValues = (arr) => {
  ...
  return onlyNums.map(num => num % 2 === 0 ? 'even' : 'odd');
};
