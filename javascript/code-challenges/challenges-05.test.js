...
const addPurchases = (arr) => {
  return arr.reduce((accum, purchase) => accum += purchase.purchasePrice, 0);
};
...
const returnNames = (arr) => {
  return arr.reduce((accum, person) => [...accum, person.name], []);
};
...
const reversedString = (str) => {
  return [...str].reduce((accum, letter) => letter + accum, '');
};
