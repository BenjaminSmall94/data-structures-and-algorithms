...
// I wrote my own sorting algorithm for practice but will use the JavaScript built in one.
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(ingredient => {
    let ingredWONum = ingredient.slice(ingredient.indexOf(' ') + 1);
    let ingredWOMeasure = ingredWONum.slice(ingredWONum.indexOf(' ') + 1);
    result.push(ingredWOMeasure);
  });
  return result;
};
