...
const alphabetizeBetter = (arr) => {
  return arr.sort((a, b) => {
    let a_lowerCase = a.toLowerCase();
    let b_lowerCase = b.toLowerCase();
...
const sortSchedule = (arr) => {
  let daysOfWeek = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
  return arr.sort((a, b) => {
    let compareDays = daysOfWeek.indexOf(a.dayOfWeek) - daysOfWeek.indexOf(b.dayOfWeek);
    if(compareDays === 0) {
      let compareStarts = a.start - b.start;
      if(compareStarts === 0) {
        return a.end - b.end;
      } else {
